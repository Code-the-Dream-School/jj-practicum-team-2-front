...
const AttendanceList = ({ sessionId }) => {
  ...
  const attendancePercentage = sessionInfo
    ? Math.round(
        (sessionInfo.totalAttendees / sessionInfo.totalParticipants) * 100,
      )
    : 0;
    ...
    <div className="p-4">
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <div className="mt-2 flex items-center space-x-4">
            <span className="text-sm text-blue-700">
              Attendance: {sessionInfo.totalAttendees}/
              {sessionInfo.totalParticipants}({attendancePercentage}%)
            </span>
          </div>
        </div>
      )}
      ...
      <div className="mt-4 p-3 bg-gray-50 rounded-lg">
        <div className="flex justify-between text-sm text-gray-600">
          ...
          <span>
            Present: {attendanceData.filter((s) => s.isPresent).length}
          </span>
          <span>
            Absent: {attendanceData.filter((s) => !s.isPresent).length}
          </span>
        </div>
      </div>
    </div>
